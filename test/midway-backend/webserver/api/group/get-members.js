...
describe('The get group members API: GET /groups/:id/members', () => {
  ...
  it('should respond 401 if not logged in', function(done) {
    this.helpers.api.requireLogin(app, 'get', `/group/api/groups/${group.id}/members`, done);
  });
  ...
  it('should respond 404 if group is not found', function(done) {
    this.helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
      ...
      requestAsMember(request(app).get(`/group/api/groups/${new ObjectId()}/members`))
        .expect(404)
        .end((err, res) => {
  ...
  it('should respond 403 if the logged in user does not have permission to get group members (group belongs to another domain)', function(done) {
    lib.group.create({
      ...
      .then(createdGroup => {
        this.helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
          expect(err).to.not.exist;
          requestAsMember(request(app).get(`/group/api/groups/${createdGroup.id}/members`))
            .expect(403)
            .end((err, res) => {
              expect(err).to.not.exist;
  ...
  describe('filtering members with email query', function() {
    it('should respond 200 with empty list if the email query is given but not a valid email', function(done) {
      this.helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
        expect(err).to.not.exist;
        requestAsMember(request(app).get(`/group/api/groups/${group.id}/members?email=invalidemail`))
          .expect(200)
          .end((err, res) => {
            expect(err).to.not.exist;
    ...
    it('should respond 200 with member filtered by email query', function(done) {
      this.helpers.api.loginAsUser(app, adminUser.emails[0], password, (err, requestAsMember) => {
        expect(err).to.not.exist;
        requestAsMember(request(app).get(`/group/api/groups/${group.id}/members?email=${adminUser.preferredEmail}`))
          .expect(200)
          .end((err, res) => {
            expect(err).to.not.exist;
  ...
  it('should respond 200 with the members list if the current user is not a domain admin', function(done) {
    this.helpers.api.loginAsUser(app, regularUser.emails[0], password, (err, requestAsMember) => {
      expect(err).to.not.exist;
      requestAsMember(request(app).get(`/group/api/groups/${group.id}/members`))
        .expect(200)
        .end((err, res) => {
          expect(err).to.not.exist;
